...
void Shutdown()
{
    ...
    RenameThread("shugacoin-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-prunedebuglogfile", _("Prune (limit) filesize of debug.log")); // FIXME.SHUGA // prune debug.log
    if (showDebug)
    {
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/shugacoin-project/shugacoin>";
    const std::string URL_WEBSITE = "<https://shugacoin.org>";
...
void ThreadImport(std::vector<fs::path> vImportFiles)
{
    ...
    RenameThread("shugacoin-loadblk");
...
void InitLogging()
{
    ...
    fPruneDebugLog = gArgs.GetBoolArg("-prunedebuglogfile", false); // FIXME.SHUGA // prune debug.log
...
bool AppInitMain()
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the "
                  "current working directory '%s'. This is fragile, because if shugacoin is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if shugacoin is started while in a temporary directory.\n",
