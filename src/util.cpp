...
const char * const BITCOIN_CONF_FILENAME = "shugacoin.conf";
const char * const BITCOIN_PID_FILENAME = "shugacoind.pid";
const char * const DEFAULT_DEBUGLOGFILE = "debug.log";

ArgsManager gArgs;
bool fPrintToConsole = false;
bool fPrintToDebugLog = true;
bool fPruneDebugLog = false; // FIXME.SHUGA // prune debug.log
...
#else
    const char* pszModule = "shugacoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Shugacoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Shugacoin";
#else
    // Unix
    return pathRet / ".shugacoin";
#endif
#endif
}
